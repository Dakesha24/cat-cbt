...
<div class="container-fluid py-4">
  ...
  <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
    <div>
      <h2 class="fw-bold text-dark mb-1">Daftar Hasil Ujian</h2>
      <p class="text-muted mb-0">Pantau progres dan buka hasil setiap ujian yang sudah dijadwalkan</p>
    </div>
    <div class="d-flex gap-2 flex-wrap">
      <a href="<?= base_url('admin/ujian') ?>" class="btn btn-outline-secondary shadow-sm">
        <i class="bi bi-journal-text me-2"></i>Kelola Ujian
      </a>
      <a href="<?= base_url('admin/jadwal-ujian') ?>" class="btn btn-primary shadow-sm">
  ...
  <div class="exam-page shadow-sm px-3 px-md-4 py-4">
    ...
    <?php if (empty($daftarUjian)): ?>
      <div class="text-center py-5">
        ...
        <p class="text-muted mb-3">Hasil akan muncul setelah peserta terdaftar dan ujian berjalan</p>
      </div>
    <?php else: ?>
              ...
              <div class="card border-0 shadow-sm h-100 exam-card cat-card">
                <div class="card-body d-flex flex-column p-0">
                  <div class="d-flex align-items-center justify-content-between px-4 pt-3 pb-2">
                    ...
                    <span class="badge bg-<?= esc($u['status_class']) ?>"><?= esc($u['status_text']) ?></span>
                  </div>
                  <div class="px-4 pb-3 flex-grow-1">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                      ...
                      <span class="exam-meta-pill school"><i class="bi bi-buildings"></i><?= esc($u['nama_sekolah'] ?? 'Sekolah') ?></span>
                      <span class="exam-meta-pill classroom"><i class="bi bi-mortarboard"></i><?= esc($u['nama_kelas'] ?? '-') ?></span>
                      <?php if (!empty($u['kode_akses'])): ?>
                        <small class="exam-code"><i class="bi bi-key me-1"></i><?= esc($u['kode_akses']) ?></small>
                      <?php endif; ?>
                    </div>
                    <p class="text-muted small mb-3 line-clamp-2"><?= esc($u['deskripsi'] ?? '') ?></p>
                    <div class="row g-2 text-center mb-3">
                      <div class="col-4"><div class="stat-box"><div class="stat-val"><?= esc($u['peserta_selesai']) ?></div><div class="stat-lbl">Selesai</div></div></div>
                      <div class="col-4"><div class="stat-box"><div class="stat-val"><?= esc($u['peserta_sedang_mengerjakan']) ?></div><div class="stat-lbl">Aktif</div></div></div>
                      <div class="col-4"><div class="stat-box"><div class="stat-val"><?= esc($u['jumlah_peserta']) ?></div><div class="stat-lbl">Total</div></div></div>
                    </div>
                    <div class="row g-2 text-center">
                      <div class="col-6"><div class="stat-box"><div class="stat-val"><?= esc($u['rata_rata_durasi_format'] ?? '-') ?></div><div class="stat-lbl">Rata-rata</div></div></div>
                      <div class="col-6"><div class="stat-box"><div class="stat-val"><?= esc($u['durasi_tercepat_format'] ?? '-') ?></div><div class="stat-lbl">Tercepat</div></div></div>
                    </div>
                  </div>
                  <div class="px-4 pb-4 mt-auto">
                    <a href="<?= base_url('admin/hasil-ujian/siswa/' . $u['jadwal_id']) ?>" class="btn btn-outline-primary w-100" style="border-radius:0">
                      <i class="bi bi-eye me-2"></i><?= $u['status_ujian'] === 'selesai' ? 'Lihat Hasil' : 'Pantau Progress' ?>
                    </a>
                  </div>
                </div>
              </div>
              ...
              <div class="card border-0 shadow-sm h-100 exam-card cbt-card">
                <div class="card-body d-flex flex-column p-0">
                  ...
                  <div class="px-4 pb-3 flex-grow-1">
                    ...
                    <div class="row g-2 text-center">
                      <div class="col-6"><div class="stat-box"><div class="stat-val"><?= esc($u['rata_rata_durasi_format'] ?? '-') ?></div><div class="stat-lbl">Rata-rata</div></div></div>
                      <div class="col-6"><div class="stat-box"><div class="stat-val"><?= esc($u['durasi_terlama_format'] ?? '-') ?></div><div class="stat-lbl">Terlama</div></div></div>
                    </div>
                  </div>
                  <div class="px-4 pb-4 mt-auto">
                    <a href="<?= base_url('admin/hasil-ujian/siswa/' . $u['jadwal_id']) ?>" class="btn btn-outline-success w-100" style="border-radius:0">
                      <i class="bi bi-eye me-2"></i><?= $u['status_ujian'] === 'selesai' ? 'Lihat Hasil' : 'Pantau Progress' ?>
                    </a>
                  </div>
                </div>
