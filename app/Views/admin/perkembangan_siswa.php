        ...
        <div id="filterBody" style="<?= $activeCount > 0 ? '' : 'display:none' ?>">
            <form method="get" action="" class="an-filter-body">
                    ...
                    <div>
                        ...
                        <select name="sekolah_id" id="fSekolah" class="form-select form-select-sm" onchange="cascadeKelas()">
                            ...
                            <?php foreach ($filterOptions['sekolah'] as $s): ?>
                                <option value="<?= $s['sekolah_id'] ?>" data-sekolah="<?= $s['sekolah_id'] ?>" <?= (int)$selectedSekolah === (int)$s['sekolah_id'] ? 'selected' : '' ?>>
                                    <?= esc($s['nama_sekolah']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="kelas_id" id="fKelas" class="form-select form-select-sm">
                            ...
                            <?php foreach ($filterOptions['kelas'] as $k): ?>
                                <option value="<?= $k['kelas_id'] ?>" data-sekolah="<?= $k['sekolah_id'] ?? '' ?>" <?= (int)$selectedKelas === (int)$k['kelas_id'] ? 'selected' : '' ?>>
                                    <?= esc($k['nama_kelas']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="d-flex gap-2 mt-4">
                    ...
                    <a href="<?= base_url('admin/perkembangan-siswa') ?>" class="btn btn-outline-secondary btn-sm">
                        Reset
                    </a>
                </div>
            </form>
...
function cascadeKelas() {
    ...
    document.querySelectorAll('#fKelas option').forEach(o => {
        if (o.value !== '') o.style.display = (!sekolahId || o.dataset.sekolah === sekolahId) ? '' : 'none';
    });
}
...
function fetchGrafikGlobal() {
    ...
    fetch('<?= base_url('admin/grafik-perkembangan-ajax') ?>?' + params.toString())
        .then(res => res.json())
        .then(resData => {
