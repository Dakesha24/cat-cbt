            ...
            <?php if (empty($variabel)): ?>
              <tr><td colspan="6">
                <div class="empty-state">
                  ...
                  <h6>Belum ada variabel</h6>
                  <p>Tambahkan variabel pertama untuk memulai</p>
                </div>
              </td></tr>
            <?php else: ?>
                  ...
                  <td>
                    <div class="d-flex gap-1">
                      <button class="btn-act"
                        onclick="editVariabel('<?= $v['variabel_id'] ?>', '<?= esc($v['nama_variabel'], 'js') ?>', '<?= esc($v['deskripsi'] ?? '', 'js') ?>')"
                        data-bs-toggle="modal" data-bs-target="#modalEdit" title="Edit">
                        <i class="bi bi-pencil"></i>
                      </button>
                      <a href="<?= base_url('admin/variabel/hapus/' . $v['variabel_id']) ?>"
                        class="btn-act btn-act-del"
                        onclick="return confirm('Hapus variabel ini? Indikator terkait akan ikut terhapus.')" title="Hapus">
...
<div class="modal fade" id="modalTambah" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow">
      ...
      <form method="post" action="<?= base_url('admin/variabel/tambah') ?>">
        <div class="modal-body px-4 py-4">
          <div class="mb-3">
            <label class="form-label small fw-semibold">Nama Variabel <span class="text-danger">*</span></label>
            <input type="text" name="nama_variabel" class="form-control" required maxlength="100" placeholder="Contoh: Pemahaman Konsep">
          </div>
          <div class="mb-0">
            <label class="form-label small fw-semibold">Deskripsi</label>
            <textarea name="deskripsi" class="form-control" rows="3" placeholder="Penjelasan singkat tentang variabel ini..."></textarea>
          </div>
        </div>
    ...
    <div class="modal-content border-0 shadow">
      ...
      <form method="post" id="formEdit">
        <div class="modal-body px-4 py-4">
          <div class="mb-3">
            <label class="form-label small fw-semibold">Nama Variabel <span class="text-danger">*</span></label>
            <input type="text" name="nama_variabel" id="edit_nama" class="form-control" required maxlength="100">
          </div>
          <div class="mb-0">
            <label class="form-label small fw-semibold">Deskripsi</label>
...
function editVariabel(id, nama, deskripsi) {
  document.getElementById('formEdit').action = '<?= base_url('admin/variabel/edit/') ?>' + id;
