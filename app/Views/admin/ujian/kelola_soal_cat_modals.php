      ...
      <form action="<?= base_url('admin/soal/edit/' . $s['soal_id']) ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body px-4 py-4">
          <div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label small fw-semibold">Jawaban</label><select name="jawaban_benar" class="form-select"><?php foreach(['A','B','C','D','E'] as $j): ?><option value="<?= $j ?>" <?= $s['jawaban_benar']==$j?'selected':'' ?>><?= $j ?></option><?php endforeach; ?></select></div>
