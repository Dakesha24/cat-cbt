...
<div class="container-fluid py-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="<?= base_url('admin/soal/' . $ujian['id_ujian']) ?>" class="btn btn-outline-secondary"><i class="bi bi-list-task me-1"></i>Kelola Soal</a>
  </div>
  ...
  <!-- Row 1: Assign Bank (Left) + Generate Paket (Right) -->
  <div class="row g-4 mb-4">
    <!-- Kolom Kiri: Assign Bank -->
    <div class="col-lg-6">
      <div class="card shadow-sm h-100">
        <div class="card-header bg-light"><h6 class="mb-0"><i class="bi bi-database me-2"></i>Sumber Tunggal Bank Soal</h6></div>
        <div class="card-body">
          <p class="text-muted small">CBT menggunakan satu bank soal sebagai sumber tunggal.</p>
            ...
            <div class="mb-3" style="max-height:250px;overflow-y:auto">
              ...
              <?php else: ?>
                ...
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="bank_ids[]" value="<?= $bank['bank_ujian_id'] ?>" id="bank_<?= $bank['bank_ujian_id'] ?>" <?= $checked ? 'checked' : '' ?>>
                  <label class="form-check-label" for="bank_<?= $bank['bank_ujian_id'] ?>">
                    <strong><?= esc($bank['nama_ujian']) ?></strong>
                    <small class="text-muted d-block"><?= esc($bank['kategori'] ?? '') ?> · <?= (int)($bank['jumlah_soal'] ?? 0) ?> soal</small>
                  </label>
                </div>
                <?php endforeach; ?>
              <?php endif; ?>
            </div>
            <button type="submit" class="btn btn-primary w-100"><i class="bi bi-save me-1"></i>Simpan Bank Terpilih</button>
          </form>
        </div>
      </div>
    </div>

    <!-- Kolom Kanan: Generate Paket (INLINE) -->
    <div class="col-lg-6">
      <div class="card shadow-sm h-100">
        ...
        <div class="card-body">
          ...
          <?php else: ?>
            ...
            <form method="post" action="<?= base_url('admin/ujian/' . $ujian['id_ujian'] . '/generate-paket/proses') ?>">
              ...
              <div class="alert alert-info py-2 mb-2 small">Validasi: <strong>Soal per Paket harus lebih kecil dari Soal Tersedia (Y &lt; N)</strong>. Overlap antar paket diperbolehkan.</div>
  ...
  <!-- Row 2: Daftar Paket (full width) -->
  <div class="card shadow-sm">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
      <h6 class="mb-0"><i class="bi bi-box-seam me-2"></i>Daftar Paket</h6>
      <?php if (!empty($paketList)): ?>
        <a href="<?= base_url('admin/ujian/' . $ujian['id_ujian'] . '/paket/hapus-semua') ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus SEMUA paket?')"><i class="bi bi-trash"></i> Hapus Semua</a>
      <?php endif; ?>
    </div>
            ...
            <tbody>
              <?php foreach ($paketList as $p): ?>
                <tr>
                  ...
                  <td><a href="<?= base_url('admin/ujian/' . $ujian['id_ujian'] . '/paket/' . $p['paket_id'] . '/hapus') ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus paket ini?')"><i class="bi bi-trash"></i></a></td>
                </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
