...
<div class="container-fluid py-4">
  ...
  <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
    <div>
      <h2 class="fw-bold text-dark mb-1">Materi</h2>
      <p class="text-muted mb-0">Topik pelajaran yang diujikan dalam soal</p>
    </div>
  ...
  <div class="card border-0 shadow-sm data-card">
    <div class="card-header bg-white border-bottom d-flex align-items-center justify-content-between py-3 px-4">
      <span class="fw-semibold text-dark">Daftar Materi</span>
      <small class="text-muted"><?= count($materi) ?> data</small>
    </div>
    <div class="card-body p-0">
      <div class="table-responsive">
        <table class="table mb-0">
          ...
          <tbody>
            <?php if (empty($materi)): ?>
              <tr><td colspan="5"><div class="empty-state"><i class="bi bi-inbox"></i><h6>Belum ada materi</h6><p>Tambahkan materi pertama untuk memulai</p></div></td></tr>
            <?php else: ?>
              <?php $no = 1; foreach ($materi as $m): ?>
                <tr>
                  <td class="text-muted"><?= $no++ ?></td>
                  <td class="td-name"><?= esc($m['nama_materi']) ?></td>
                  <td class="td-desc"><?= esc($m['deskripsi'] ?? '—') ?></td>
                  <td style="text-align:center"><span class="count-pill"><?= $m['jumlah_soal'] ?? 0 ?></span></td>
                  <td>
                    <div class="d-flex gap-1">
                      <button class="btn-act"
                        onclick="editMateri('<?= $m['materi_id'] ?>', '<?= esc($m['nama_materi'], 'js') ?>', '<?= esc($m['deskripsi'] ?? '', 'js') ?>')"
                        data-bs-toggle="modal" data-bs-target="#modalEdit" title="Edit">
                        <i class="bi bi-pencil"></i>
                      </button>
                      <a href="<?= base_url('guru/materi/hapus/' . $m['materi_id']) ?>"
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow">
      <div class="modal-header bg-primary text-white px-4 py-3">
        <h5 class="modal-title fw-semibold"><i class="bi bi-plus-circle me-2"></i>Tambah Materi</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form method="post" action="<?= base_url('guru/materi/tambah') ?>">
        <div class="modal-body px-4 py-4">
          <div class="mb-3">
            <label class="form-label small fw-semibold">Nama Materi <span class="text-danger">*</span></label>
            <input type="text" name="nama_materi" class="form-control" required maxlength="200" placeholder="Contoh: Hukum Newton">
          </div>
          <div class="mb-0">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow">
      <div class="modal-header bg-warning text-dark px-4 py-3">
        <h5 class="modal-title fw-semibold"><i class="bi bi-pencil me-2"></i>Edit Materi</h5>
      ...
      <form method="post" id="formEdit">
        <div class="modal-body px-4 py-4">
          <div class="mb-3">
            <label class="form-label small fw-semibold">Nama Materi <span class="text-danger">*</span></label>
            <input type="text" name="nama_materi" id="edit_nama" class="form-control" required maxlength="200">
          </div>
          <div class="mb-0">
            <label class="form-label small fw-semibold">Deskripsi</label>
...
<script>
function editMateri(id, nama, deskripsi) {
  document.getElementById('formEdit').action = '<?= base_url('guru/materi/edit/') ?>' + id;
